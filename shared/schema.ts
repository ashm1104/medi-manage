import { pgTable, text, uuid, boolean, timestamp, date, numeric, pgEnum } from "drizzle-orm/pg-core";
...
export const caseStatusEnum = pgEnum("case_status", ["OPEN", "HOLD", "CLOSED"]);
...
export const patient_facilities = pgTable("patient_facilities", {
  ...
  facility_id: uuid("facility_id").references(() => facilities.id).notNull(),
...
export const ack_docs = pgTable("ack_docs", {
  ...
  visiting_doc_share: text("visiting_doc_share"),
...
export const insertAckDocSchema = createInsertSchema(ack_docs)
  .omit({ id: true, owner_user_id: true, created_at: true })
  .extend({
    ack_no: z.string().regex(/^ACK-\d{8}-\d{4}$/, "Invalid ack_no format"),
  });
...
export type InsertPatientFacility = z.infer<typeof insertPatientFacilitySchema>;
